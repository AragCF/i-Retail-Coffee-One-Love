...
set "SRC_DIR=vendotek_tty_logs"
set "DST_DIR=test_reports\vendotek_tty_access"
...
set "LATEST="
for /f "delims=" %%F in ('dir /b /a-d /o-d "%SRC_DIR%\VENDOTEK_TTY_ACCESS_*.zip" 2^>nul') do if not defined LATEST set "LATEST=%%F"
if not defined LATEST (
  echo [ERROR] No VENDOTEK_TTY_ACCESS_*.zip found in %SRC_DIR%
  pause
  exit /b 3
)
...
git commit -m "Add Vendotek tty access probe %LATEST%"
if errorlevel 1 (
...
echo.
echo [SUCCESS] Latest Vendotek tty access result is now in Git.
echo %LATEST%
pause
exit /b 0
