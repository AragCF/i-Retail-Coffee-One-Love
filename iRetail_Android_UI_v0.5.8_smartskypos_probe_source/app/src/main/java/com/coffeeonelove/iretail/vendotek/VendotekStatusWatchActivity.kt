...
/**
 * Non-financial Vendotek readiness watch.
 *
 * Intended to be run after a real power-cycle of the screenless Vendotek VT-class terminal.
 * The activity sends only IDL and IDL+SystemInformation=STATUS, never VRP/FIN/ABR/DIS.
 */
class VendotekStatusWatchActivity : Activity() {
    ...
    private fun buildUi() {
        ...
        root.addView(TextView(this).apply {
            text = "Vendotek VTK — readiness watch"
            textSize = 20f
            setTextColor(Color.BLACK)
        })
        root.addView(TextView(this).apply {
            text = "Без оплаты: только IDL и STATUS с интервалом не менее 10 секунд."
            textSize = 13f
    ...
    private fun runProbe() {
        ...
        var writes = 0
        try {
            line("READY_WATCH_BEGIN uid=${android.os.Process.myUid()} tty=$TTY_PATH")
            if (!VtkCodec.selfTest()) {
                errorMarker("VTK_CODEC_SELFTEST_FAILED")
                return
            }
            line("VTK_CODEC_SELFTEST_OK")
            ...
            if (idlResponse == null) {
                errorMarker("VTK_READY_WATCH_IDL_TIMEOUT")
                return
            }
            line("VTK_IDL_OK operation=${idlResponse.ascii(VtkCodec.TAG_OPERATION_NUMBER) ?: "-"}")

            var finalStatus = "<none>"
            var consecutiveCleanReady = 0
            var serviceSamples = 0
            var validStatusSamples = 0

            for (index in 1..MAX_STATUS_SAMPLES) {
                rateLimit(lastTxAt)
                val request = VtkCodec.buildSystemInfo(localTime(), "STATUS")
                line("STATUS_TX sample=$index bytes=${request.size} hex=${VtkCodec.hex(request)}")
                ...
                val info = response?.ascii(VtkCodec.TAG_SYSTEM_INFO)
                if (info == null) {
                    consecutiveCleanReady = 0
                    line("STATUS_SAMPLE sample=$index info=<timeout-or-missing>")
                    continue
                }

                validStatusSamples++
                finalStatus = info
                val upper = info.uppercase(Locale.US)
                val cleanReady = upper.contains("STATE=READY") &&
                    !upper.contains("NOT_READY") &&
                    !upper.contains("BUSY") &&
                    !upper.contains("DISABLED")
                val inService = upper.contains("IN_SERVICE")
                if (inService) serviceSamples++
                consecutiveCleanReady = if (cleanReady) consecutiveCleanReady + 1 else 0

                line(
                    "STATUS_SAMPLE sample=$index cleanReady=$cleanReady inService=$inService " +
                        "info=${safe(info)}"
                )

                if (consecutiveCleanReady >= 2) {
                    successMarker(
                        "VTK_READY_STABLE sample=$index consecutive=$consecutiveCleanReady status=${safe(info)}"
                    )
                    return
                }
            }

            if (validStatusSamples > 0 && serviceSamples == validStatusSamples) {
                line(
                    "VTK_SERVICE_STATE_PERSISTENT samples=$serviceSamples status=${safe(finalStatus)}"
                )
            }
            successMarker(
                "VTK_READY_WATCH_COMPLETE validSamples=$validStatusSamples serviceSamples=$serviceSamples " +
                    "finalStatus=${safe(finalStatus)}"
            )
        } catch (e: Exception) {
            errorMarker(
                "VTK_READY_WATCH_ERROR ${e.javaClass.simpleName}:${safe(e.message)} " +
                    "cause=${e.cause?.javaClass?.simpleName ?: "-"}:${safe(e.cause?.message)}"
            )
        } finally {
            try { outputStream?.close() } catch (_: Exception) {}
            try { input?.close() } catch (_: Exception) {}
            line("READY_WATCH_END writes=$writes financialCommands=0")
        }
    }
        ...
        while (SystemClock.elapsedRealtime() < deadline && !Thread.currentThread().isInterrupted) {
            ...
            for (raw in extracted.first) {
                ...
                if (requireSystemInfo && info == null) {
                    line("STATUS_PLAIN_IDL_ACK operation=${frame.ascii(VtkCodec.TAG_OPERATION_NUMBER) ?: "-"}")
                    continue
                }
    ...
    companion object {
        ...
        private const val MAX_STATUS_SAMPLES = 8
    }
}
