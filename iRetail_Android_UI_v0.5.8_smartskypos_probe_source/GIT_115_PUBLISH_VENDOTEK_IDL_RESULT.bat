...
set "SRC_DIR=vendotek_idl_logs"
set "DST_DIR=test_reports\vendotek_idl"
...
set "LATEST="
for /f "delims=" %%F in ('dir /b /a-d /o-d "%SRC_DIR%\VENDOTEK_IDL_*.zip" 2^>nul') do if not defined LATEST set "LATEST=%%F"
if not defined LATEST (
  echo [ERROR] No VENDOTEK_IDL_*.zip found in %SRC_DIR%
  pause
  exit /b 3
)
...
git commit -m "Add Vendotek IDL smoke result %LATEST%"
if errorlevel 1 (
...
echo.
echo [SUCCESS] Latest Vendotek IDL smoke result is now in Git.
echo %LATEST%
pause
exit /b 0
