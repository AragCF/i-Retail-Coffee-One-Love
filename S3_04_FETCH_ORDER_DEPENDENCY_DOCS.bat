...
echo ============================================================
echo i-Retail v0.5.49 - S3 ORDER DEPENDENCY DOCUMENTATION AUDIT
echo ============================================================
echo [1/2] Fetch related API documentation.
echo [2/2] Package and publish evidence to Git.
echo.

powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0tools\iRetailOrderDependencyDocsAudit.ps1" -RepoRoot "%CD%"
if errorlevel 1 (
  echo [ERROR] S3 dependency documentation audit failed.
  pause
  exit /b 20
)

call "%~dp0S3_05_PUBLISH_ORDER_DEPENDENCY_DOCS.bat"
if errorlevel 1 (
...
echo.
echo [SUCCESS] S3 dependency documentation audit completed and published.
powershell -NoProfile -Command "Write-Host 'Документация зависимостей заказа в Git, продолжай.'"
pause
exit /b 0
