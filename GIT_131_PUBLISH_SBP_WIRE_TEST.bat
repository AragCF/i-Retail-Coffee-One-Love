...
if /I not "%GIT_BRANCH%"=="v0.5.114-sbp-wire-sanitizer" (
  echo [ERROR] Wrong branch: %GIT_BRANCH%
  exit /b 2
)

set "LATEST_ZIP="
for /f "delims=" %%F in ('dir /b /a:-d /o:-d "sbp_wire_logs\SBP_WIRE_TEST_*.zip" 2^>nul') do if not defined LATEST_ZIP set "LATEST_ZIP=%CD%\sbp_wire_logs\%%F"
...
set "DEST_DIR=%CD%\test_reports\sbp_wire_test"
...
git add -- "test_reports/sbp_wire_test/%ZIP_NAME%" "test_reports/sbp_wire_test/%ZIP_NAME%.safe.txt" "test_reports/sbp_wire_test/%ZIP_NAME%.sha256.txt"
if errorlevel 1 exit /b 9
git diff --cached --quiet
if not errorlevel 1 exit /b 0
git commit -m "test: SBP synthetic AOA wire %ZIP_NAME%"
if errorlevel 1 exit /b 10
git push origin HEAD
if errorlevel 1 exit /b 11
echo [SUCCESS] Safe SBP wire report committed and pushed.
exit /b 0
