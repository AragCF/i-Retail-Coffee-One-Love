...
set "LATEST_FILE="
for /f "delims=" %%F in ('dir /b /a-d /o-d "test_reports\s3_dry_run_v2\S3_DRY_RUN_V2_*.zip" 2^>nul') do if not defined LATEST_FILE set "LATEST_FILE=%%F"
if not defined LATEST_FILE (
  echo [ERROR] No S3_DRY_RUN_V2_*.zip found.
  pause
  exit /b 20
)

set "LATEST_REL=test_reports\s3_dry_run_v2\%LATEST_FILE%"
set "LATEST_ZIP=%CD%\%LATEST_REL%"

powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0tools\Assert-S3DryRunV2Safe.ps1" -ZipPath "%LATEST_ZIP%"
if errorlevel 1 (
  echo [ERROR] S3 DRY_RUN v2 ZIP safety validation failed.
  pause
  exit /b 21
)
...
echo [SUCCESS] Latest S3 DRY_RUN v2 ZIP was committed and pushed.
exit /b 0
