...
if /I not "%GIT_BRANCH%"=="v0.5.108-acquirer-readonly-diagnostics" (
  echo [ERROR] Wrong branch: %GIT_BRANCH%
  exit /b 2
)

set "LATEST_ZIP="
for /f "delims=" %%F in ('dir /b /a:-d /o:-d "acquirer_snapshot_logs\ACQUIRER_SNAPSHOT_*.zip" 2^>nul') do if not defined LATEST_ZIP set "LATEST_ZIP=%CD%\acquirer_snapshot_logs\%%F"
...
set "DEST_DIR=%CD%\test_reports\acquirer_snapshot"
...
git add -- "test_reports/acquirer_snapshot/%ZIP_NAME%" "test_reports/acquirer_snapshot/%ZIP_NAME%.safe.txt" "test_reports/acquirer_snapshot/%ZIP_NAME%.sha256.txt"
if errorlevel 1 exit /b 9
git diff --cached --quiet
if not errorlevel 1 exit /b 0
git commit -m "test: acquirer read-only snapshot %ZIP_NAME%"
if errorlevel 1 exit /b 10
git push origin HEAD
if errorlevel 1 exit /b 11
echo [SUCCESS] Safe acquirer snapshot committed and pushed.
exit /b 0
