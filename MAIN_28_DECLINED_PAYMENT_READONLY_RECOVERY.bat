...
if /I not "%EXPECTED_VERSION%"=="0.5.107-declined-readonly-recovery" (
...
echo ============================================================
echo i-Retail %EXPECTED_VERSION% - DECLINED PAYMENT READ-ONLY RECOVERY
echo ============================================================
echo SAFETY:
echo   real POS remains FALSE
echo   no PAYMENT / CANCEL / REFUND / reconciliation is sent
echo   only PING, INFO, GET_STATE, GET_TERMINAL_DATA,
echo   GET_LAST_TRANSACTION and GET_TRANSACTION are allowed
echo ============================================================
echo.
...
if "%KOZEN_ADB_AVAILABLE%"=="1" (
  ...
  echo [KOZEN ADB] unavailable - allowed. Existing production bridge will be reached over USB/AOA.
)
...
if not exist "%APP_APK%" (
  echo [ERROR] APK not found: %APP_APK%
  pause
  exit /b 21
)
...
echo [4/7] Starting READ-ONLY transaction recovery...
adb -s "%JL22%" shell am start -W -n com.coffeeonelove.iretail/.ui.MainActivity --es machine_mode standalone --ez real_pos_enabled false --ez fiscal_declined_payment_recovery true >nul
if errorlevel 1 (
  echo [ERROR] Could not start read-only recovery.
  pause
  exit /b 24
)

set "WAIT_LOG=%TEMP%\iretail_declined_recovery_wait.log"
...
for /l %%S in (1,1,150) do (
  ...
  adb -s "%JL22%" logcat -d -v brief DeclinedPaymentRecovery:V IretailKozenClient:V IretailMachineMode:I *:S > "%WAIT_LOG%" 2>&1
  findstr /C:"RECOVERY_RESULT ok=true" "%WAIT_LOG%" >nul 2>nul
  if not errorlevel 1 (
    set "OUTCOME=RECOVERY_OK"
    goto RECOVERY_DONE
  )
  findstr /C:"RECOVERY_RESULT ok=false" "%WAIT_LOG%" >nul 2>nul
  if not errorlevel 1 (
    set "OUTCOME=RECOVERY_FAILED"
    goto RECOVERY_DONE
  )
  findstr /C:"RECOVERY_REJECTED" "%WAIT_LOG%" >nul 2>nul
  if not errorlevel 1 (
    set "OUTCOME=RECOVERY_REJECTED"
    goto RECOVERY_DONE
  )
  timeout /t 1 /nobreak >nul
)
...
if not exist "declined_recovery_logs" mkdir "declined_recovery_logs"
set "OUT=declined_recovery_logs\DECLINED_PAYMENT_RECOVERY_%TS%"
mkdir "%OUT%"

echo [6/7] Collecting and sanitizing read-only evidence...
(
...
adb -s "%JL22%" logcat -d -v threadtime DeclinedPaymentRecovery:V IretailKozenClient:V IretailMachineMode:I AndroidRuntime:E *:S > "%OUT%\RAW_jl22_logcat.txt" 2>&1
if "%KOZEN_ADB_AVAILABLE%"=="1" (
...
(
  ...
  echo %OUTCOME%
  echo.
  findstr /I "RECOVERY_RESULT RECOVERY_FAILED RECOVERY_REJECTED READ_ONLY_RECOVERY_OK READ_ONLY_RECOVERY_FAILED" "%OUT%\RAW_jl22_logcat.txt"
...
echo [7/7] Creating archive and publishing safe evidence...
...
call "%~dp0GIT_128_PUBLISH_DECLINED_READONLY_RECOVERY.bat"
if errorlevel 1 (
  echo [WARN] Report publication failed; local ZIP is preserved.
)

echo.
echo ============================================================
echo READ-ONLY DECLINED PAYMENT RECOVERY COMPLETE
echo Outcome: %OUTCOME%
