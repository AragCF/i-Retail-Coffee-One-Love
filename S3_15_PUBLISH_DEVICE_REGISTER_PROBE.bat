...
if not defined LATEST_FILE (
  echo [ERROR] No S3_DEVICE_REGISTER_*.zip found.
  pause
  exit /b 20
)

set "LATEST_REL=test_reports\s3_device_register\%LATEST_FILE%"
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0tools\Assert-S3DeviceRegisterProbeSafe.ps1" -ZipPath "%CD%\%LATEST_REL%"
if errorlevel 1 (
...
git commit -m "test: controlled S3 device register probe %STAMP%"
if errorlevel 1 (
...
echo [SUCCESS] Latest device/register report was committed and pushed.
exit /b 0
