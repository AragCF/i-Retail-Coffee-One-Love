...
checks = {
    ...
    "separate redaction and hard-secret sets": "$redactionSecrets = @(" in ps and "$hardReportSecrets = @(" in ps and "$requestRedactionSecrets" in ps,
    "device code excluded from hard value scan": "[string]$config.device_code" in ps and "$hardReportSecrets" in ps and "$allSecrets" not in ps,
    "hard scan message names credential/token": "credential/token value found in report" in ps,
    "structured validator covers username": "username|phone|email" in safe_ps,
    "sanitizer catches token suffixes": "token$" in ps and "password$" in ps and "secret$" in ps,
    "ZIP validator catches arbitrary token fields": '(token|password|secret)' in safe_ps,
    "unneeded profile identifiers redacted": "account_id$" in ps and "user_id$" in ps and "offline_shop_id$" in ps,
    ...
    "generic lists converted with ToArray": "$profileServiceItems.ToArray()" in ps and "$usedServiceItems.ToArray()" in ps,
    "no generic-list array subexpression in summary": "profile_services=@($profileServiceItems)" not in ps and "used_services=@($usedServiceItems)" not in ps,
...
if failed:
    raise SystemExit("S3 device/service reconciliation guard failed: " + ", ".join(failed))

print(f"[OK] S3 device/service reconciliation guard: {len(checks)} checks passed")
