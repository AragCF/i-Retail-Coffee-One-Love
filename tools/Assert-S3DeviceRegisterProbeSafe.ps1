...
try {
    ...
    if ($summary.order_send_allowed -ne $false) {
        Write-Host "[FAIL] order_send_allowed must be false."
        exit 12
    }
    if ($summary.config_update_allowed -ne $false) {
        Write-Host "[FAIL] config_update_allowed must be false."
        exit 13
    }
    if ($summary.register_retry_allowed -ne $false) {
        Write-Host "[FAIL] register_retry_allowed must be false."
        exit 14
    }

    if ($summary.mode -eq "CONTROLLED_REGISTER_PROBE") {
        if ([int]$summary.register_calls -ne 1) {
            Write-Host "[FAIL] controlled probe must record exactly one register call."
            exit 15
        }
        ...
        if ([int]$summary.register_calls -ne 0) {
            Write-Host "[FAIL] read-only recovery must record zero register calls."
            exit 17
        }
    } else {
    ...
    Write-Host "[OK] S3 device/register ZIP safety validation passed."
    exit 0
}
finally {
    $archive.Dispose()
