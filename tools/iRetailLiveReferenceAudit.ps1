...
function Is-SensitiveKey([string]$Key,[bool]$RedactPersonName = $false) {
    ...
    if ($RedactPersonName -and $Key -match '(?i)^name
    ...
    if ($Value -is [System.Collections.IDictionary]) {
        ...
        foreach ($k in $Value.Keys) {
            $result[[string]$k] = Sanitize-Value $Value[$k] ([string]$k) $RedactPersonName
        }
    ...
    if ($Value -is [System.Collections.IEnumerable] -and -not ($Value -is [string])) {
        $items = @()
        foreach ($item in $Value) { $items += ,(Sanitize-Value $item "" $RedactPersonName) }
        return $items
    }
    ...
    if ($Value -is [pscustomobject]) {
        ...
        foreach ($prop in $Value.PSObject.Properties) {
            $result[$prop.Name] = Sanitize-Value $prop.Value $prop.Name $RedactPersonName
        }
        return [pscustomobject]$result
    }
    ...
    foreach ($request in $requests) {
        ...
        if (Test-Path -LiteralPath $rawPath) {
            ...
            try {
                ...
                $safePayload = Sanitize-Value $obj "" $redactPersonName
