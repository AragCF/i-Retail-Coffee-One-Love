...
function Is-SensitiveKey([string]$Key,[bool]$RedactPersonName = $false) {
    if ([string]::IsNullOrWhiteSpace($Key)) { return $false }
    if ($RedactPersonName -and $Key -match '(?i)^name$') { return $true }
...
function Sanitize-Value($Value,[string]$Key = "",[bool]$RedactPersonName = $false) {
    if (Is-SensitiveKey $Key $RedactPersonName) {
    ...
    if ($Value -is [System.Collections.IDictionary]) {
        ...
        foreach ($k in $Value.Keys) {
            $dict[[string]$k] = Sanitize-Value $Value[$k] ([string]$k) $RedactPersonName
        }
    ...
    if ($Value -is [System.Collections.IEnumerable] -and -not ($Value -is [string])) {
        ...
        foreach ($item in $Value) {
            $items += ,(Sanitize-Value $item "" $RedactPersonName)
        }
        return $items
    }
    ...
    if ($Value -is [pscustomobject]) {
        ...
        foreach ($prop in $Value.PSObject.Properties) {
            $obj[$prop.Name] = Sanitize-Value $prop.Value $prop.Name $RedactPersonName
        }
...
try {
    ...
    foreach ($request in $requests) {
        ...
        if (Test-Path -LiteralPath $rawPath) {
            ...
            try {
                $responseObject = Get-Content -Raw -LiteralPath $rawPath -Encoding UTF8 | ConvertFrom-Json
                $redactPersonName = ($request.name -eq "employees_by_channel")
                $safePayload = Sanitize-Value $responseObject "" $redactPersonName
    ...
    $http2xxCount = @($results | Where-Object {
        $code = 0
        [void][int]::TryParse([string]$_.http_code,[ref]$code)
        $code -ge 200 -and $code -lt 300
    }).Count
    $jsonParsedCount = @($results | Where-Object { $_.json_parsed }).Count
    ...
    $summary = [ordered]@{
        ...
        http_2xx=$http2xxCount
        json_parsed=$jsonParsedCount
    ...
    $summaryLines = @(
        ...
        "HTTP2xx=" + $http2xxCount,
        "JsonParsed=" + $jsonParsedCount,
        "OrderSendAllowed=NO"
    )
    ...
    Write-Host ("[ENDPOINTS] " + $http2xxCount + "/" + $requests.Count + " HTTP 2xx")
    Write-Host ("[JSON] " + $jsonParsedCount + "/" + $requests.Count + " parsed")
    exit 0
}
finally {
