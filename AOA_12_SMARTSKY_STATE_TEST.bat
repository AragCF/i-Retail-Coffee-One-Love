...
echo ============================================================
echo i-Retail READ-ONLY SmartSkyPOS state over USB/AOA
echo ============================================================
echo JL22 : %JL22%
echo Kozen: %KOZEN%
echo.
...
echo [2/8] Installing Kozen Bridge v0.2...
...
echo [3/8] Installing JL22 AOA host v0.2...
...
echo [6/8] Starting JL22 USB/AOA SmartSkyPOS state probe...
...
echo.
echo Expected final line:
echo   SMARTSKYPOS getState works through JL22 USB AOA Kozen Bridge
echo.
...
if not exist "aoa_state_logs" mkdir "aoa_state_logs"
set "OUT=aoa_state_logs\AOA_STATE_JL22_KOZEN_%TS%"
mkdir "%OUT%"
...
(
  ...
  echo kozen=%KOZEN%
  echo timestamp=%TS%
  echo test=AOA_12_SMARTSKY_STATE_TEST
  echo safety=READ_ONLY_GET_STATE
) > "%OUT%\00_info.txt"
...
adb -s "%KOZEN%" shell dumpsys package com.coffeeonelove.iretail.kozenbridge > "%OUT%\KOZEN_06_bridge_package.txt" 2>&1
adb -s "%JL22%" shell dumpsys package com.coffeeonelove.iretail.aoahost > "%OUT%\JL22_06_host_package.txt" 2>&1

(
  echo ===== JL22 RESULT =====
  findstr /I "AOA_PROTOCOL AOA_START AOA_BULK_READY AOA_PING_OK AOA_LINK_OK GET_STATE SMARTSKY_STATE_OVER_AOA" "%OUT%\JL22_02_aoa_logcat.txt"
  echo.
  echo ===== KOZEN BRIDGE RESULT =====
  findstr /I "BRIDGE_READY SMARTSKY_BOUND SMARTSKY_BIND SMARTSKY_GET_STATE RX GET_STATE TX STATE" "%OUT%\KOZEN_02_bridge_logcat.txt"
  echo.
  echo ===== USB IDS =====
...
echo TEST COMPLETE.
echo Publish with:
echo   call GIT_102_PUBLISH_AOA_STATE_RESULT.bat
echo ============================================================
pause
exit /b 0
